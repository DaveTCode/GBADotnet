...
connection.on("SendDevice", function (device) {
    ...
    if (isSavingState) {
        localStorage.setItem(loadedRomGuid + "-" + isSavingState, device);
        isSavingState = 0;
    }
});
...
document.addEventListener("keyup", function (e) {
    ...
    } else if (e.key.toLowerCase() === "1") { // Save State
        isSavingState = 1;
        connection.invoke("GetState");
    } else if (e.key.toLowerCase() === "2") { // Load State
        var state = localStorage.getItem(loadedRomGuid + "-" + isSavingState);

        if (state) {
            connection.invoke("LoadState", state);
        }
    }
});
